...
4. Items with score greater than or equal to `60` set `is_selected = 1` for full-content fetching.
...
6. When usable content exists, the database `pipeline_state` becomes `fetched`.
...
RSS → raw → scored → fetched → API/UI projection → UI
...
- API/UI projection: the API derives `ready`, `translated`, or `translation_failed` from Chinese fields and `has_translate_failed`.
- UI: The frontend displays API/UI status without reading database pipeline fields.
