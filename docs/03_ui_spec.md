...
- `translated`：翻译完成；UI 展示中文标题、中文摘要、中文正文、来源、发布时间和评分。`translated` 数据必须包含非空 `title`、`summary_zh`；详情页还必须包含非空 `content_zh`。
...
- 不提供高级设置。
- 不提供信息源分类。
- 信息源列表只渲染 `GET /api/sources` 返回的未删除信息源；UI 不读取也不展示内部 `is_deleted` 字段。
- 启用 / 禁用控件只能调用 `PATCH /api/sources/{id}`，不得新增频率、分类或高级配置。
- 禁用信息源仍保留在配置页并可重新启用。
- 禁用全部信息源被后端拒绝时，必须展示接口返回的错误状态。
- 删除信息源只能调用 `DELETE /api/sources/{id}`；删除成功后重新加载列表，因 API 不再返回该 source，必须从列表中视觉移除。
...
| SourceRow | 展示、启用 / 禁用并删除 RSS 信息源 | 用户无法管理已配置输入源 |
